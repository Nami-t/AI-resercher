...
class ResearchStateInput(TypedDict):from itertools import chain
from re import search
...
def summerize_results(state: ResearchState):
    model=ChatOllama(model="deepseek-r1:8b")
    prompt= ChatPromptTemplate.from_template(summary_template)
    chain =print | model
...
builder.add_node("search_web",search_web)
builder.add_node("summerize_results", summerize_results)
builder.add_node("generate_response",generate_response)
...
st.title("AI RESEARCHER")
query=st.text_input("Enter your researach query : ")
